...
def wyrok(url2):
  openai.api_key = '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'
  completion = openai.ChatCompletion.create( model="gpt-3.5-turbo", messages=[ {"role": "user", "content": "Jaką karę wymierzono:"+ url2} ] )
  time.sleep(3)
  reply = completion.choices[0].message.content
  print(reply)
  return reply

def podsumowanie(url2):
  openai.api_key = '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'
  completion = openai.ChatCompletion.create( model="gpt-3.5-turbo", messages=[ {"role": "user", "content": "Przedstaw krótkie podsumowanie tego linku:"+ url2} ] )
  time.sleep(3)
  reply = completion.choices[0].message.content
  print(reply)
  return reply
